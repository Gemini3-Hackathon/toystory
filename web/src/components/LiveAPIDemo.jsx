...
const LiveAPIDemo = () => {
  ...
  const [projectId, setProjectId] = useState(
    localStorage.getItem("projectId") && localStorage.getItem("projectId") !== "gemini-live-demo-1772251398" ? localStorage.getItem("projectId") : "aiden-lab-19644"
  );
  const [model, setModel] = useState(
    localStorage.getItem("model") && localStorage.getItem("model") !== "gemini-2.0-flash-exp" ? localStorage.getItem("model") : "gemini-live-2.5-flash-native-audio"
  );
  ...
  const [videoInstruction, setVideoInstruction] = useState(
    `이제 카메라를 통해 주변환경을 너에게 보여주고 있어.
보이는 사물의 정보를 JSON 형식으로 출력해줘. 자연어 응답없이 바로 아래 형식으로 답해
---
{
"object":str,
"color":str,
"number of object":int
}`
  );
  ...
  const connect = async () => {
    ...
    try {
      ...
      clientRef.current.responseModalities = [responseModality];
      ...
      try {
        ...
        if (videoStreamerRef.current) {
          ...
          if (videoInstruction.trim() !== "") {
            addMessage(`[Video Prompt Sent]: ${videoInstruction}`, "system");
            clientRef.current.sendTextMessage(videoInstruction);
          }
        } else {
          addMessage("[Connect to Gemini first]", "system");
        }
          ...
          <div className="dropdown">
            ...
            <div className="dropdown-content config-dropdown">
              ...
              <div className="control-group">
                ...
                <div className="input-group">
                  <label>Video Prompt (sent when camera starts):</label>
                  <textarea
                    rows="3"
                ...
                <div className="input-group">
                  ...
                  >
                    ...
                    <option value="TEXT">Text Only (JSON, etc.)</option>
                  </select>
                </div>
                <div className="input-group">
                  <label>Voice (Audio mode only):</label>
                  <select
                    value={voice}
